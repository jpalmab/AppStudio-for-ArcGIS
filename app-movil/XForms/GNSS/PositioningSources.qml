...
Item {
    ...
    LoggingCategory {
        ...
        name: AppFramework.typeOf(sources, true)
    }
    ...
    onNetworkHostSelected: {
        console.log(logCategory, "Connecting to remote host:", hostname, "port:", port);
    ...
    onDeviceSelected: {
        console.log(logCategory, "Connecting to device:", device.name, "address:", device.address, "type:", device.deviceType, device);
